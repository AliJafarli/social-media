package com.texnoera.socialmedia.security.filter;
...
public class JwtRequestFilter extends OncePerRequestFilter {
        ...
        if (authHeader.isPresent() && authHeader.get().startsWith(BEARER_PREFIX)) {
            ...
            try {
                if (!jwtTokenProvider.validateToken(jwt)) {
                    throw new ExpiredJwtException(null, null, ExceptionConstants.TOKEN_EXPIRED.getUserMessage());
                }
    ...
    private void handleTokenExpiration(String requestURI, String jwt, HttpServletResponse response) throws IOException {
        if (isAuthEndpoint(requestURI)) {
            ...
            sendErrorResponse(response, HttpStatus.UNAUTHORIZED, ExceptionConstants.TOKEN_EXPIRED.getUserMessage());
        }
    }

    private void handleSignatureException(HttpServletResponse response) throws IOException {
        sendErrorResponse(response, HttpStatus.UNAUTHORIZED, ExceptionConstants.INVALID_TOKEN_SIGNATURE.getUserMessage());
    }

    private void handleUnexpectedException(HttpServletResponse response, Exception e) throws IOException {
        log.error(ExceptionConstants.ERROR_DURING_JWT_PROCESSING.getUserMessage(), e);
        sendErrorResponse(response, HttpStatus.INTERNAL_SERVER_ERROR, ExceptionConstants.UNEXPECTED_ERROR_OCCURRED.getUserMessage());
    }
